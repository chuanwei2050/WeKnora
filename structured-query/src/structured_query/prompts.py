...
4. 括号限定词首先用于业务类别和字段消歧，不得自动拼入 ILIKE；仅当同一候选值连续含主体+限定词才可整体复制，否则用候选支持的主体标准词并保留限定词消歧。
5. 多近似类别时只选与限定词+完整业务名最一致的一个；不得用 OR 同时覆盖用户原词、纠正词及相近其他类别（除非用户/Profile 明确等价）。
6. 主体与限定词在库中被隔开时，对同一字段用多个 AND ILIKE；输出前检查：去掉 `%`/`_` 后每个连续片段必须能从对应字段的某条 Example/候选值中连续复制。unsupported_value_literal → 换更短候选子串。

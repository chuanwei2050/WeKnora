...
- 超过 100 MiB 的 DOCX 转换不占用浏览器主线程和 HTTP 请求生命周期。
...
### 1. 文件大小决定 DOCX 即时渲染或部分异步转换

DOCX 原文件不超过 100 MiB 时沿用项目现有 `docx-preview`，并继续执行 ZIP 中央目录安全检查；超过 100 MiB 时浏览器不得下载原文件进行排版，而是进入异步部分预览。ZIP64、目录损坏、单条目声明异常或压缩比超过 100 直接拒绝转换并只提供文本降级。

选择 100 MiB 是为了完整复用项目原有预览能力，同时为超大文档设置清晰边界；同步 DOM 排版期间事件循环无法可靠取消，因此超过该边界不得尝试浏览器渲染。
...
### 3. Worker 在受控子进程中生成前 30 页 PDF

Worker 从知识库对应存储读取原文件到独立临时目录，使用 `exec.CommandContext` 启动 `soffice --headless --convert-to pdf`，固定导出第 1–30 页。原文件超过 256 MiB 时进入超大文档单并发队列，允许最大 1 GiB 输入、4 GiB 声明解压量和50000个压缩条目，并设置900秒及256 MiB部分PDF上限。转换后验证PDF头、大小和页数（不得超过30页），再写入原知识库对应对象存储。

选择前30页PDF而不是完整PDF：浏览器DOM数量稳定，LibreOffice能较好保持版式，同时避免长期生成和传输完整产物。完整内容统一通过原文件下载查看。
...
- `GET /api/v1/knowledge/:id/preview-artifact`：仅当当前内容版本对应的部分预览任务成功时流式返回前30页PDF，支持Range。

每个接口都先按当前租户读取知识条目并复用知识库 Viewer 权限校验；不能仅凭 task ID 或对象路径读取产物。错误响应使用稳定错误码供前端展示和决定是否降级。

### 5. 前端只管理任务，不执行复杂文档排版

DOCX超过100 MiB后，前端不再下载完整原文用于渲染，而是提交前30页任务并轮询。成功后展示部分PDF和“完整内容请下载原文件”提示；失败时保留重试、文本分块和下载入口。组件卸载只停止轮询，不取消共享任务。

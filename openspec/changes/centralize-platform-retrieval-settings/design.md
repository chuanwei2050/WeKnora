...
平台默认值为：查询扩展开启、`EmbeddingTopK=30`、两路 recall 各 `50`、`RRFVectorWeight=0.7`、向量阈值 `0.3`、关键词阈值 `0.3`、`RerankCandidateTopK=20`、`RerankTopK=10`、Rerank 阈值 `0.3`、批量检索总结果上限 `200`、批量检索正文字符上限 `200000`。校验必须保证：

- recall 值为 `1..500`；
- `1 <= RerankTopK <= RerankCandidateTopK <= EmbeddingTopK`；
- RRF 权重和阈值处于既有合法范围；
- 批量总结果上限和正文字符上限为正数且处于服务端允许范围；
...
### 4. 批量响应使用全局预算而不新增单条配置

平台策略增加 `BatchMaxResults` 和 `BatchMaxContentChars`，仅作为 `POST /api/integration/v1/rag/search-batch` 的整批响应保护。batch 中每条 query 仍使用平台 `RerankTopK`，并可由 `queries[].top_k` 继续缩小；不增加语义重复的批量单条 rerank 配置。

批量搜索先完成各查询既有召回和重排，再按请求顺序执行稳定预算分配：以轮询方式从每个 query 依次选择结果，在预算允许时优先覆盖更多 query；候选按 `knowledge_version_id + chunk_id` 全局去重，身份字段缺失时回退到 `knowledge_id + chunk_id`。字符预算按 Unicode 字符计数且只计算 `content`；系统不截断单条证据，放不进剩余字符预算的候选直接跳过并继续检查其他候选。

返回仍使用现有逐 query `results` 结构，不引入引用对象或要求调用方解引用。结果数和正文字符预算均为硬上限；预算不足或证据已由其他 query 保留时，部分成功 query 的 `results` MAY 为空。服务端必须记录裁剪前后数量、正文字符数和受影响 query 数，供调用方继续治理查询数量。
...
- [批量查询数超过总结果上限] → 轮询优先覆盖更多 query，但严格遵守硬上限；调用方仍应限制查询数量并处理空结果。
- [单条正文超过剩余字符预算] → 不截断证据正文，跳过放不下的候选并继续选择其他结果。
...
5. 增加 Integration batch 整批预算与公平分配，并在平台 UI 暴露两个总量配置。
6. 观察有效配置、召回数量和结果数量日志；确认后停止记录旧覆盖值的运行时使用。

...
- [x] 1.4 增加 client 显式管理员绑定、项目级外部身份唯一键和外部身份启用状态迁移，并兼容已有映射升级
...
- [x] 3.6 覆盖 ticket 首次兑换、错误 origin、重放、过期、并发消费、角色提升、会话续期、CSRF 缺失和退出测试
- [x] 3.7 实现普通用户首次自动创建、管理员仅绑定现有账号、项目身份停用/启用及对应会话撤销
...
- [x] 4.5 执行共享身份源、项目本地身份源、跨租户、跨 client、错误 Origin、撤销传播和权限收回安全测试
...
- [x] 4.9 编写外挂知识库对接文档，覆盖项目配置、账号映射、知识库菜单、悬浮窗、认证、接口、权限和停用同步

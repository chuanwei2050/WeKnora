...
`POST /api/v1/integration/knowledge/read` is an additive service-client endpoint requiring `knowledge:read` and the existing knowledge-base allowlist. Request fields: `knowledge_base_id`, `knowledge_id`, `updated_at` (from inventory), `folder_ids` (optional), `page` (1–1000000), `page_size` (1–50).

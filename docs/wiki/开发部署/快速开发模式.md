...
```bash
bash ./scripts/quick-dev.sh start   # 或省略参数，默认 start
bash ./scripts/quick-dev.sh stop
```

`start` 会自动启动基础设施、后端和前端；后端、前端日志分别写入 `logs/backend.log` 和 `logs/frontend.log`，进程 PID 写入 `tmp/backend.pid` 和 `tmp/frontend.pid`。`stop` 会按 PID 文件停止本脚本上次拉起的前后端，并停止基础设施容器。

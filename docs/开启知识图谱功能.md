...
本文档介绍如何在 KnowledgeMesh 中启用并验证知识图谱（Neo4j）功能，帮助你完成从环境准备到前端配置的全流程。

## 前置条件

- 已完成 KnowledgeMesh 后端与前端的基础部署。

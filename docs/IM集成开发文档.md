...
KnowledgeMesh 的 IM 集成模块将企业即时通讯平台（企业微信、飞书、Slack、Telegram、钉钉、Mattermost）接入 KnowledgeMesh 知识问答管道，支持在 IM 中直接向 AI 提问并获得实时流式回答。
...
保存后 KnowledgeMesh 会自动建立到企业微信的 WebSocket 长连接。日志中出现以下内容表示连接成功：
...
2. 填写：
   - **URL**：粘贴上一步复制的回调地址
   - **Token**：填入在 KnowledgeMesh 中设置的 Token
   - **EncodingAESKey**：填入在 KnowledgeMesh 中设置的 EncodingAESKey
3. 点击保存，企业微信会发送 GET 验证请求，KnowledgeMesh 会自动响应
...
2. **请求地址**：粘贴从 KnowledgeMesh 复制的回调地址
3. 添加事件 `im.message.receive_v1`
4. 点击保存时飞书会发送 URL 验证请求（challenge），KnowledgeMesh 会自动响应
...
2. 开启 **Enable Events** 开关。
3. 在 **Request URL** 中粘贴从 KnowledgeMesh 复制的回调地址。Slack 会发送一个 challenge 请求，KnowledgeMesh 会自动响应并验证通过。
...
2. **消息接收地址**：粘贴从 KnowledgeMesh 复制的回调地址
...
Mattermost 为**自建部署**，当前仅支持 **Webhook** 模式：**出站 Webhook（Outgoing Webhook）** 将用户消息 POST 到 KnowledgeMesh，Bot 通过 **REST API v4** 发回频道/线程回复。与 Slack Events API 类似，回调需快速返回 `200`，实际回答由 Bot Token 异步调用接口完成。

> 需要公网或内网可达的回调 URL（Mattermost 服务器能访问 KnowledgeMesh 的 `/api/v1/im/callback/{channel_id}`）。若 KnowledgeMesh 在内网，请在 Mattermost 中配置 [Trusted Internal Connections](https://docs.mattermost.com/configure/environment-configuration-settings.html) 或将回调地址加入允许列表。
...
3. **Content Type** 建议选择 **application/json**（也支持 `application/x-www-form-urlencoded`，KnowledgeMesh 两种均可解析）。
4. 选择触发频道，或配置**触发词**（与频道组合规则以 Mattermost 说明为准）。
5. **Callback URLs** 先留空或填占位；保存后在 KnowledgeMesh 创建渠道后会得到正式地址。
...
在绑定频道发送触发出站 Webhook 的消息，应收到 KnowledgeMesh 的回复；回复默认出现在**触发帖所在线程**（使用 Mattermost 的 `root_id` 与触发 `post_id` 对齐）。
...
│                  ▼                                                           │
│   ┌──────────────────────────────────────┐                                   │
│   │     KnowledgeMesh Core (QA Pipeline)       │     核心层                       │
...
将 IM 渠道中的用户会话映射到 KnowledgeMesh 会话：
...
每个 IM 渠道代表一个 IM 平台机器人与 KnowledgeMesh Agent 的绑定关系。一个 Agent 可以绑定多个渠道（如同时接入企业微信、飞书、Slack 与 Mattermost），同一平台也可以创建多个渠道（如不同的企业微信机器人）。
...
将 IM 渠道 (渠道 ID + 用户 + 群聊) 映射到 KnowledgeMesh 会话，实现对话上下文持续性。首次交互自动创建，后续消息复用同一会话。并发创建通过唯一约束 + fallback 查询处理。存储于 `im_channel_sessions` 表。
...
仅支持 **Webhook** 模式：Mattermost **Outgoing Webhook** 将请求体 POST 到 KnowledgeMesh 统一回调地址；适配器实现 `Adapter`、`StreamSender`、`FileDownloader`（标准库 HTTP 调用 REST API，无第三方 SDK）。

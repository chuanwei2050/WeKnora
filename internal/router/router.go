...
func RegisterIntegrationPublicRoutes(r *gin.Engine, h *handler.IntegrationHandler) {
	...
	integration.GET("/knowledge-bases/folders", h.ListKnowledgeBaseFolders)
...
func RegisterKnowledgeRoutes(r *gin.RouterGroup, handler *handler.KnowledgeHandler) {
	...
	k := r.Group("/knowledge")
	{
		...
		k.POST("/:id/preview-generation", handler.RequestDocumentPreview)
		k.GET("/:id/preview-generation", handler.GetDocumentPreviewStatus)
		k.GET("/:id/preview-generated", handler.GetGeneratedDocumentPreview)
		// 更新图像分块信息
...
func RegisterAuthRoutes(r *gin.RouterGroup, handler *handler.AuthHandler) {
	...
	r.POST("/auth/bidreview-sso", handler.BidReviewSSO)

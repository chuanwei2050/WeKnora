...
// SplitTabularRows splits line-oriented tabular text (Excel KV rows) into one
// chunk per data line. Lines starting with "## " are sheet headers and are
// prepended to following data rows so each indexed chunk keeps sheet context.
// Oversized lines are force-split to respect cfg.ChunkSize.
func SplitTabularRows(text string, cfg SplitterConfig) []Chunk {
	chunkSize := cfg.ChunkSize
	if chunkSize <= 0 {
		chunkSize = 512
	}
	...
	flushLine := func(line string, lineStart int) {
		lineRuneLen := runeLen(line)
		prefix := ""
		if sheetPrefix != "" {
			prefix = sheetPrefix
		}
		// Only force-split the data line; re-attach sheet prefix on every piece
		// so continuation chunks keep sheet context and offsets stay in-document.
		if lineRuneLen <= chunkSize {
			chunks = append(chunks, Chunk{
				Content: prefix + line,
				Seq:     len(chunks),
				Start:   lineStart,
				End:     lineStart + lineRuneLen,
			})
			return
		}
		for _, part := range forceSplitRunes(line, lineStart, chunkSize) {
			chunks = append(chunks, Chunk{
	...
	for i, line := range lines {
		lineStart := runePos
		// Account for the newline separator except after the last segment.
		lineRuneLen := runeLen(line)
		if i < len(lines)-1 {
			runePos += lineRuneLen + 1
		} else {
			runePos += lineRuneLen
		}

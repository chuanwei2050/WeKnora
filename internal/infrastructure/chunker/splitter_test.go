...
func TestSplitTabularRows_OneChunkPerDataRow(t *testing.T) {
	text := "## 人员清单\n序号: 1,姓名: 许乃汉,职称专业: 系统集成项目管理工程师\n序号: 2,姓名: 夏雨欣,职称专业: 系统集成项目管理师\n## 统计\n证书: 系统集成项目管理工程师,人数: 1\n"
	cfg := SplitterConfig{ChunkSize: 512, ChunkOverlap: 0}
	chunks := SplitTabularRows(text, cfg)
	if len(chunks) != 3 {
		t.Fatalf("expected 3 data-row chunks, got %d: %#v", len(chunks), chunks)
	}
	if !strings.Contains(chunks[0].Content, "## 人员清单\n") {
		t.Fatalf("sheet header should be prepended: %q", chunks[0].Content)
	}
	if !strings.Contains(chunks[0].Content, "许乃汉") {
		t.Fatalf("first row missing name: %q", chunks[0].Content)
	}
	if strings.Contains(chunks[0].Content, "夏雨欣") {
		t.Fatalf("rows must not be merged: %q", chunks[0].Content)
	}
	if !strings.Contains(chunks[2].Content, "## 统计\n") {
		t.Fatalf("stats sheet header missing: %q", chunks[2].Content)
	}
}

func TestSplitTabularRows_OversizedLineKeepsSheetPrefix(t *testing.T) {
	longCell := strings.Repeat("证书内容", 80) // well over small chunk size
	text := "## SheetA\n姓名: 张三,详情: " + longCell + "\n"
	cfg := SplitterConfig{ChunkSize: 64, ChunkOverlap: 0}
	chunks := SplitTabularRows(text, cfg)
	if len(chunks) < 2 {
		t.Fatalf("expected force-split into multiple chunks, got %d", len(chunks))
	}
	docLen := utf8.RuneCountInString(text)
	for i, c := range chunks {
		if !strings.HasPrefix(c.Content, "## SheetA\n") {
			t.Fatalf("chunk[%d] missing sheet prefix: %q", i, c.Content)
		}
		if c.Start < 0 || c.End > docLen || c.Start >= c.End {
			t.Fatalf("chunk[%d] bad offsets start=%d end=%d docLen=%d", i, c.Start, c.End, docLen)
		}
	}
}

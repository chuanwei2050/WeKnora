...
func TestImageVLMFailureReturnsErrorOnLastAttempt(t *testing.T) {
	enqueuer := &imageFinalizeEnqueuer{}
	service := &ImageMultimodalService{taskEnqueuer: enqueuer}
	...
	err := service.handleVLMFailure(context.Background(), payload, "OCR", errors.New("vlm unavailable"), true)
	if err == nil {
		t.Fatal("expected permanent failure error on last attempt")
	}
	// Finalization is deferred to Handle; handleVLMFailure itself must not enqueue.
	if len(enqueuer.tasks) != 0 {
		t.Fatalf("handleVLMFailure must not finalize directly, got %+v", enqueuer.tasks)
	}
}

func TestImageMultimodalFinalizeEnqueuesPostProcess(t *testing.T) {
	enqueuer := &imageFinalizeEnqueuer{}
	service := &ImageMultimodalService{taskEnqueuer: enqueuer}
	payload := types.ImageMultimodalPayload{
		TenantID:        1,
	...
	if err := service.checkAndFinalizeAllImages(context.Background(), payload); err != nil {
		t.Fatal(err)
	}

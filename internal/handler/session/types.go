...
// AnalyzeKnowledgeTableRequest runs a bounded batch of fact lookups against
// one tenant-authorized structured knowledge file.
type AnalyzeKnowledgeTableRequest struct {
	KnowledgeID string                     `json:"knowledge_id" binding:"required"`
	Queries     []types.TableAnalysisQuery `json:"queries" binding:"required,min=1,max=100"`
	MaxRows     int                        `json:"max_rows" binding:"omitempty,min=1,max=50"`

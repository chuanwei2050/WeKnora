...
func TestBuildFinalAnswerPromptUsesPublicOutputContract(t *testing.T) {
	...
	for _, required := range []string{"只输出面向用户的最终答案", "不要输出分析过程", "谁持有证书？", "physical table names", "internally generated aliases", "business column names", "natural language", "structured-query process"} {

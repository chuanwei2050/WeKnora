...
func TestExecutePreservesGeneratedProjectionAndPredicate(t *testing.T) {
	...
	tool := &DataAnalysisTool{
		BaseTool:  dataAnalysisTool,
		db:        db,
		sessionID: "preserve-sql-test",
		loadedSchemas: map[string]*TableSchema{"people": {TableName: "people", Columns: []ColumnInfo{
	...
	payload, _ := json.Marshal(DataAnalysisInput{
		Action:      DataAnalysisActionExecute,
		KnowledgeID: "people",
		Sql:         `SELECT 姓名 FROM people WHERE 证书分类 LIKE '%系统集成项目管理工程师%'`,
	})
	result, err := tool.Execute(ctx, payload)
	if err != nil {
	...
	if len(rows) != 1 || rows[0]["姓名"] != "许乃汉" {
		t.Fatalf("query predicate was changed: %#v", rows)
	}
	if _, exists := rows[0]["证书分类"]; exists {
		t.Fatalf("query projection was changed: %#v", rows)
	}
}

...
def setup(doc, short_title):
    ...
    font(footer.add_run("商业交付培训资料  ·  2026-09-01"), 8, color=GRAY)
...
def cover(doc, title, subtitle, audience):
    ...
    info = table(doc, [("适用对象", audience), ("产品版本", "客户交付版（以现场安装包为准）"),
                       ("文档版本", "V1.2"), ("保密等级", "客户内部资料"),
                       ("编制日期", "2026 年 9 月 1 日"), ("客户环境", "____________________________")],
...
def user_manual():
    ...
    para(doc, "版本治理：如需开放普通成员投稿，在知识库编辑页打开“知识版本治理”，设置投稿范围（仅管理员、全部普通用户或指定贡献者），并按需指定审核人。", bold_prefix="版本治理：")
    bullets(doc, ["启用投稿后，普通成员提交的文件先进入草稿或待审批状态；审核通过后才开始解析、索引和图谱构建，已发布内容在新版本审核期间继续可用。"])

    heading(doc, "8. 导入与维护知识")
    para(doc, "文件夹组织：文档知识库左侧依次显示“全部文档”“未分类”“公共知识”和普通文件夹，选择文件夹后右侧只显示该范围内的文档。", bold_prefix="文件夹组织：")
    bullets(doc, ["“公共知识”是固定容器，不能直接承载文档；具备编辑权限时可在其下新建公共子文件夹。", "普通一级文件夹可新建直属二级文件夹；文件夹层级最多两级，二级文件夹不能继续新建子级。", "文件夹的“搜索”开关只在检索入口显式启用文件夹过滤时生效；当前问答、@ 文件选择和支持该参数的接口会按此过滤，全局“搜索”页面以该入口配置为准。关闭父文件夹搜索时，受控过滤请求也会排除其子文件夹内容。", "上传、单个移动或批量移动文档时选择具体文件夹；拖拽排序只允许在同一父级内调整顺序。"])
    ...
    bullets(doc, ["支持上传文件夹；上传前先在左侧选择具体目标文件夹。"])
    ...
    bullets(doc, ["为文档设置清晰标题、标签和文件夹；同类资料使用统一命名。", "文件夹重命名、删除和排序应由有编辑权限的用户执行；只有整棵子树为空时才能递归删除，含文档的文件夹或子树删除前应先处理内容。", "文件更新后重新上传或触发数据源同步，避免新旧版本并存。", "删除前确认是否仍被智能体或业务接口使用。", "抽样检查解析正文、召回片段和引用来源，而不是只看“上传成功”。"])
    ...
    bullets(doc, ["知识库范围可选全部、指定或不使用；指定模式应按最小权限选择。", "在“设置 → 检索策略”维护平台级预算：查询扩展、RRF 融合保留数量、向量/关键词预召回数量与权重、向量/关键词阈值、Rerank 候选/保留数量与阈值。", "平台级检索策略供问答、智能体和接口共用；Integration 批量检索还受总结果数和正文字符数上限保护。没有有效证据时，按“回答兜底”策略返回固定内容或交给模型生成。", "Agent 模式按任务开放知识检索、Wiki、网络搜索、MCP 和 Skills；工具越多，误调用面和审核成本越高。", "图片、语音、附件和 @ 按钮应与模型能力、文件类型及业务场景一致。", "系统 Prompt 写清角色、可用数据、禁止事项、引用要求和失败处理，不写入真实密钥。"])
    ...
    bullets(doc, ["定期查看用户反馈，按知识缺失、召回错误、生成错误或权限问题分类。", "版本治理开启时，投稿人可提交或撤销自己的投稿；审核人处理待审批稿的通过或驳回。只有当前已发布、可检索且未失效的版本进入回答、引用和图谱检索，旧版本不再参与召回。", "图三元组审核应核对实体、关系、来源片段和业务 Schema；错误关系拒绝或修正后再写入图谱。", "审核结论应能追溯到知识来源和处理人。"])
    ...
    bullets(doc, ["能够新增、编辑、停用租户并解释删除限制。", "能够进入指定租户管理用户与知识库范围。", "能够完成模型、搜索、解析、向量、存储和 MCP 的连接测试。", "能够配置并测试平台智能体，理解启停影响。", "能够配置知识版本治理并完成一次投稿、审批和发布验证。"])
    heading(doc, "14.2 租户管理员", 2)
    bullets(doc, ["能够新增/禁用用户并按最小权限授权知识库。", "能够创建并完整配置文档型、问答型或 Wiki 知识库。", "能够管理 FAQ、数据源、反馈和图三元组审核。", "能够使用固定问题集完成一次验收评测。", "能够创建两级文件夹、设置搜索开关，并完成一次文档批量移动。"])
    heading(doc, "14.3 普通成员", 2)
    bullets(doc, ["能够登录并识别当前角色/租户。", "能够检索授权知识库并打开来源片段。", "能够用 @ 指定知识库发起问答并核对引用。", "能够选择合适智能体并识别无答案或越权请求。", "能够通过列表状态标签识别草稿、待审批和已驳回文档；对自己的投稿点击“提交”并确认状态变为“待审批”。"])
...
def deployment_manual():
    ...
    bullets(doc, ["常驻服务均为 running 或 healthy。", "App /health 成功，Web UI 可打开并登录。", "立即修改默认管理员密码。", "配置一组可用的对话、Embedding、ReRank 模型并通过连接测试。", "上传脱敏测试文档，确认解析、索引、搜索、问答和引用全链路成功。", "确认对象进入目标存储，检索引擎无连接错误。", "如启用文件夹能力，创建一级和二级文件夹并确认禁止第三级；创建公共子文件夹，并确认“公共知识”固定容器不能直接承载文档。", "如启用知识版本治理，提交一份投稿，确认待审批版本不进入检索；审批通过后完成解析/索引，并确认回答只引用当前已发布版本。", "启用 Langfuse 时，问答后能看到 Trace。", "启用 Neo4j 时，图谱抽取和查询通过。"])
    ...
    bullets(doc, ["向量索引未备份时，按知识库执行重新索引；图谱启用时按所用数据库工具恢复或重建。", "恢复后验证登录、文档列表、对象下载、检索、问答和引用，不以容器启动作为成功标准。", "同时核对文件夹父子关系、文件夹搜索开关和当前发布版本指针，避免恢复后出现目录孤儿或旧版本重新进入召回。", "明确 RPO、RTO、备份频率、保留周期、备份加密和恢复责任人；至少每季度演练一次。"])
    ...
    table(doc, [("Compose 报必填变量为空", "补齐 .env，重新执行 docker compose config --quiet"),
                ...
                ("磁盘快速增长", "定位卷、日志、对象与索引；按保留策略清理，先备份再操作"),
                ("文件夹内容检索范围异常", "核对父子文件夹归属、搜索开关及请求是否启用受控文件夹过滤；再用固定问题集复测"),
                ("投稿文件未进入回答", "核对知识版本治理开关、投稿状态、审核结果和当前发布版本；审批通过后再检查解析与索引任务")], [2600, 6760], headers=["现象", "处理路径"])

    heading(doc, "12. 上线检查表")
    bullets(doc, ["资源、NTP、DNS、证书、端口和防火墙已确认。", "交付包 SHA-256、镜像摘要和实际部署模式已记录。", "所有默认密钥已替换，.env 已受控保存。", "Compose 配置解析通过，镜像版本已记录。", "健康检查和端到端业务验收通过。", "管理员密码已修改，角色和知识库授权符合最小权限。", "备份任务已配置，并完成一次恢复演练。", "监控告警、日志保留、应急联系人及升级回滚责任人已登记。", "升级后的数据库迁移已完成；既有普通文件夹保持根级，二级关系、公共子文件夹和文档归属无孤儿记录。", "如启用版本治理，已验收投稿、撤销、通过、驳回，以及当前发布版本的检索隔离。"])

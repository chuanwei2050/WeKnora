export default {
  ...
  graphSettings: {
    ...
    description: 'Extract entities and relations from documents into the knowledge graph (distinct from the Wiki page graph).',
    ...
    rebuildAwaitingReview: 'Extraction finished. {n} triples await review before writing to the knowledge graph.',
    ...
    templateDescription: 'Use a template to fill extraction rules, then refine them in advanced settings.',
    ...
    requireTripleReviewDescription: 'When enabled, formal extraction enters a review queue and writes to the graph only after approval. Default off. Try-extract never queues.',
    openTripleReview: 'Open Triple Review',
    rulesLabel: 'Ingest & Graph Search Rules',
    rulesDescription: 'Only relation-bearing text is extracted; Tags/entity_types constrain graph writes. Whether to search the graph at Q&A time is decided automatically by the system — this page has no graph-search toggle.',
    ...
    defaultsHint: 'Templates only prefill the schema and few-shot example; they are never bound automatically. You can also configure everything without a template.',
    sampleTextLabel: 'Few-shot Sample Text',
    sampleTextDescription: 'Optionally paired with a complete expected output. Incomplete examples such as entities without a type are not injected. Try extraction can regenerate the output.',
    ...
    entityListDescription: 'Expected entities for the sample text, not entities already stored in the graph.',
    ...
    relationListDescription: 'Expected relations for the sample text, not relations already stored in the graph.',
    ...
    extractActionsLabel: 'Try Extraction',
    extractActionsDescription: 'Test the sample with the current schema. This only updates the few-shot output and never writes to the graph database.',

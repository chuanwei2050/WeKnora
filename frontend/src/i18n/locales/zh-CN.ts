export default {
  ...
  graphSettings: {
    title: "知识图谱配置",
    description: "从文档中抽取实体和关系并写入知识图谱（与 Wiki 页面图谱不同）。",
    enableLabel: "启用实体关系提取",
    enableDescription: "开启后对新处理或重建的文档生效。",
    ...
    requireTripleReviewDescription: "审核通过后再写入图数据库。",
    openTripleReview: "进入三元组审核",
    rulesLabel: "入库与搜图规则",
    rulesDescription: "入图范围由上方模式控制；Tags/entity_types 约束入图。问答是否搜图由系统根据问题自动判断，本页不提供搜图开关。",
    generalExtractionTemplate: "通用抽取（不限领域）",
    softwareTestingTemplate: "软件测评",
    applyTemplate: "应用模板",
    templateLoaded: "图谱模板已应用，所有字段仍可继续编辑",
    defaultsHint: "模板只用于填充初始 Schema 和 few-shot 示例，不会自动绑定知识库；也可以不使用模板直接配置",
    sampleTextLabel: "Few-shot 示例文本",
    sampleTextDescription: "可选的抽取示范，不会直接写入图数据库。",
    sampleTextPlaceholder: "输入一段与知识库领域一致、包含明确实体和关系的自然语言文本...",
    generateRandomText: "根据关系类型生成测试文本",
    entityListLabel: "期望实体（Few-shot 输出）",
    entityListDescription: "示例文本对应的期望实体；它们不是图谱中已经入库的实体",
    ...
    relationListDescription: "示例文本对应的期望关系；它们不是图谱中已经入库的关系",
    selectEntity: "选择实体",
    ...
    extractActionsDescription: "仅测试当前配置，不写入图数据库。",
    ...
    extractSummary: "试运行完成：保留 {entities} 个实体、{relations} 条关系。结果已经过当前白名单、数量上限和置信度过滤。",
    exampleLoaded: "示例已加载",
  ...
  knowledgeEditor: {
    ...
    chunking: {
      ...
      inheritHeadingDescription: "切块时将 Markdown 章节标题写入后续块正文，提升检索命中。仅对新导入/重建生效，不会自动全库重嵌。",

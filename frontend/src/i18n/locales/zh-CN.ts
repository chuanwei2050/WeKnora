export default {
  ...
  graphSettings: {
    ...
    ingestionModeDescription: "完整抽取召回更高；关系信号模式仅处理命中关系关键词的文本，可降低模型调用量",
    ...
    tagsLabel: "关系类型（白名单）",
    tagsDescription: "Tags 为关系类型白名单：非空时入库/试抽取会丢弃未知关系类型",
    tagsPlaceholder: "输入关系类型，如：uses、tests、supports",
    generateRandomTags: "生成随机标签",
    entityTypesLabel: "实体类型（白名单）",
    entityTypesDescription: "entity_types 为实体类型白名单；严格模式下未知或空类型将被拒绝",
    entityTypesPlaceholder: "输入实体类型，如：tool、metric、defect",
    strictSchemaLabel: "严格 schema",
    strictSchemaDescription: "开启后要求 Tags 与 entity_types 均非空，并硬过滤未知实体类型",
    requireTripleReviewLabel: "三元组需人工审核",
    requireTripleReviewDescription: "开启后正式抽取先入审核队列，通过后才写入图库；默认关闭。试抽取不入队。",
    rulesLabel: "入库与搜图规则",
    rulesDescription: "入图范围由上方模式控制；Tags/entity_types 约束入图。问答是否搜图由系统根据问题自动判断，本页不提供搜图开关。",
    loadSoftwareTestingPreset: "加载软件测评预设",
    presetLoaded: "已加载软件测评实体/关系预设",
    defaultsHint: "新建或首次启用抽取时，默认填入软件测评 Tags / entity_types，并开启严格 schema",
